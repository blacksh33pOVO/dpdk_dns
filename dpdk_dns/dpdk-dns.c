...
static void zdns_init_port(struct rte_mempool *mbuf_pool) {
	...
	if (rte_eth_tx_queue_setup(global_portid, 0, 512, rte_eth_dev_socket_id(global_portid), NULL) < 0) {
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		unsigned num_recvd = rte_eth_rx_burst(global_portid, 0, mbuf, BURST_SIZE);
		...
		for (i = 0;i < num_recvd;i ++) {  // udp : 53
			...
			if (iphdr->next_proto_id == IPPROTO_UDP) {
				...
				rte_eth_tx_burst(global_portid, 0, mbuf+i, 1);
